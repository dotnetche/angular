...
export class LayoutComponent implements OnInit, OnDestroy {
    ...
    scheme: 'dark' | 'light';
    theme: string;
    public set navigation(value: FuseNavigationItem[]) {
        this._navigation = value;
        this._navigationChanged.emit(value);
    }

    public get navigation(): FuseNavigationItem[] {
        return this._navigation;
    }

    public get navigationChanged(): Observable<FuseNavigationItem[]> {
        return this._navigationChanged;
    }

    private _navigation: FuseNavigationItem[] = [];
    private _navigationChanged: EventEmitter<FuseNavigationItem[]>;
    ...
    constructor(
        ...
        @Inject('ITranslateService') translateService: ITranslateService,
        @Inject('IPermissionsService') permissionService: IPermissionsService,
        @Inject('ISecurityService') securityService: ISecurityService
    ) {
        this.translateService = translateService;
        this.permissionService = permissionService;
        this.securityService = securityService;
        this._navigationChanged = new EventEmitter<FuseNavigationItem[]>();
        this.securityService.isAuthenticatedEvent.subscribe(isAuthenticated => {
            if (isAuthenticated) {
                this._navigationChanged.emit(this.navigation);
            }
        });
    }
    ...
    ngOnInit(): void {
        ...
        MainNavigationUtil.getNavigation(this.permissionService, this.translateService).then(navigation => {
            if (navigation != undefined) {
                this.navigation = navigation.futuristic;
            }
        });
    }
